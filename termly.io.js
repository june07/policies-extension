import('https://policies.june07.com/js/fileNameHeuristics.js')
.then(module => {
    ...
    function prependButton() {
        let button = document.querySelector('[class^=policy-embeder-copy-button-]'),
            policiesButton = document.querySelector('[id=policies_june07_com-extension-button]')
        if (button && !policiesButton) {
            ...
            button.insertAdjacentElement('beforebegin', policiesButton)
            prependedButton = true
            policiesButton.addEventListener('click', event => {
                document.querySelector('[class^=policy-embeder-format-content-body-]').innerText
                ...
                let content = document.querySelector('[class^=policy-embeder-format-content-body-]').innerText
                policy = {
                    name: module.getFileNameFromDocument(content),
                    content
                }
    ...
    window.addEventListener('click', event => {
        if (event.target.innerHTML.match(/add\sto\swebsite/i)) {
